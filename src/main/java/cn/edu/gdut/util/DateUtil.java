...
public class DateUtil {
	...
	public static final double MIN = 1000 * 60;
	...
	public static Date parse(String date) {
		...
		try {
			...
			try {
				...
				try {
					return getFormat(DAYFORMAT).parse(date);
	...
	public static double minDiff(Date d1, Date d2) {
		if(d1==null||d2==null){
			return 0;
		}
		long timeStamp = d1.getTime() - d2.getTime();
		return Math.abs(timeStamp / MIN);
	}
	...
	public static List<String> parseDates(String startDate, String endDate,
			int intervalDate) {
		List<String> dates = new ArrayList<String>();
		dates.add(startDate);
		while (true) {
			startDate = format(
					getDateAfter(parse(startDate, DAYFORMAT), intervalDate),
					DAYFORMAT);
			dates.add(startDate);
			if (parse(startDate, DAYFORMAT)
					.compareTo(parse(endDate, DAYFORMAT)) == 0) {
				break;
			} else if (parse(startDate, DAYFORMAT).after(
					parse(endDate, DAYFORMAT))) {
